...
class ChickGUI:
    ...
    def __init__(self, cav):
        ...
        # cav->fm1+fm2
        # fm1->fm11+fm12
        self.fm1 = tk.Frame(cav, width=700, bg="")

        # 使用OpenCV读取要播放的视频文件
        self.videoCap = cv2.VideoCapture(self.pathVideo)
        
        # fm11 显示视频
        self.fm11 = tk.Frame(self.fm1, width=700, height=450, bg="")
        tk.Label(self.fm11, text="视频", font=("Arial", 16)).\
        pack(expand=tk.YES, side=tk.TOP, pady=5, padx=5, anchor=tk.W)
        # image1 = Image.OPEN(r'test.jpg') # C:\\code\\intelligencering\\c\
        # hicken_video_demo\\chicken_video_demo\\chicken_demo\\
        ...
        '''
        self.scale = tk.Scale(self.fm12, from_=0, to=100, orient=tk.HORIZONTAL, command=None) # command: define function
        self.scale.set(self.value)  # 设置初始值
        self.scale.pack(fill=tk.X, expand=1, side=tk.LEFT, padx=5, ipady=10)
        '''
        self.fm12.pack(pady=5, fill=tk.BOTH)
        self.fm1.pack(side=tk.LEFT, padx=10, pady=10, fill=tk.BOTH)
        
        # fm2
        self.fm2 = tk.Frame(cav, width=500, bg="")
        ...
        #进度条以及完成程度  
        self.out_rec1 = self.canvas1.create_rectangle(5,5,400,25,outline = "blue",width = 1)  
        self.fill_rec1 = self.canvas1.create_rectangle(5,5,5,25,outline = "",width = 0,fill = "blue")  
        ...
        #进度条以及完成程度  
        self.out_rec2 = self.canvas2.create_rectangle(5,5,400,25,outline = "blue",width = 1)  
        self.fill_rec2 = self.canvas2.create_rectangle(5,5,5,25,outline = "",width = 0,fill = "blue")  
        ...
        #进度条以及完成程度  
        self.out_rec3 = self.canvas3.create_rectangle(5,5,400,25,outline = "blue",width = 1)  
        self.fill_rec3 = self.canvas3.create_rectangle(5,5,5,25,outline = "",width = 0,fill = "blue")  
        ...
        #进度条以及完成程度  
        self.out_rec4 = self.canvas4.create_rectangle(5,5,400,25,outline = "blue",width = 1)  
        self.fill_rec4 = self.canvas4.create_rectangle(5,5,5,25,outline = "",width = 0,fill = "blue")  
        ...
        #进度条以及完成程度  
        self.out_rec5 = self.canvas5.create_rectangle(5,5,400,25,outline = "blue",width = 1)  
        self.fill_rec5 = self.canvas5.create_rectangle(5,5,5,25,outline = "",width = 0,fill = "blue")  
